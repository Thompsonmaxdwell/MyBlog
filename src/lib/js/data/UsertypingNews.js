...
import {form_Post, post_textArea ,post_title,upload_img, news_card,uid} from './userPostElement'
    ...
    static Upload_news_to_Database(){
        ...
        let  storageRef = firebase.storage().ref('photo/' + file.name);
                   ...
                   db.collection('upload_news').add({
                        news_title: post_title.value,
                        news_content : post_textArea.value,
                        news_img: url,
                        news_uid :uid.value
                  });
               })
        ...
        db.collection('upload_news').onSnapshot(snapshot=>{
           ...
           changes.forEach(element => {
                ...
                if(element.type == 'added'){
                    append_list_news(element.doc.data().news_img,  element.doc.data().news_title, element.doc.data().news_content, element.doc.id);
                    readPostWhenIsClick();

                }else if(element.type == 'removed'){   
                    let data_id = news_card.querySelector('[data_id =' + element.doc.id + ']');
                     news_card.removeChild(data_id);
                }
